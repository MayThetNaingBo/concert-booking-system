...
// ── Layout config ─────────────────────────────────────────
const GAP     = 4;    // px gap between seat tiles
const ARC_DEG = 320;  // seats fill 320° — 40° entrance gap centred at the bottom
...
// How many seats fit in a ring given its circumference
function seatsForRing(radius, tileW) {
  return Math.max(2, Math.floor(((ARC_DEG * Math.PI) / 180 * radius) / (tileW + GAP)));
}

// Pre-compute position for every seatNumber (e.g. "A1", "K58")
function buildPositionMap() {
  const map = {};
  ZONES.forEach(zone => {
    zone.rows.forEach(({ row, radius, tileW, tileH }) => {
      const count    = seatsForRing(radius, tileW);
      const startDeg = -90 - ARC_DEG / 2;   // symmetrically centred on top
      ...
      <div className="cd-page">
        ...
        <div className="zone-pills">
          {ZONES.map(z => (
            <div key={z.id} className="zone-pill">
              <span className="zone-dot" style={{ background: z.color.base }}/>
              {z.label}
        ...
        <div style={{ display:"flex", justifyContent:"center" }}>
          <svg viewBox="0 0 940 980" width="min(940px,98vw)" height="min(980px,98vw)" style={{ display:"block", overflow:"visible" }}>
            ...
            {ZONES.map((z, zi) => {
              ...
              return <circle key={z.id+"-a"} cx={SVG_CX} cy={SVG_CY} r={(o+i2)/2} fill="none" stroke={z.color.base} strokeWidth={o-i2} opacity={0.045}/>;
            })}
            ...
            {ZONES.map((z, zi) => {
              if (zi === 0) return null;
              return <circle key={z.id+"-sep"} cx={SVG_CX} cy={SVG_CY} r={z.rows[0].radius - 17}
            ...
            {seats.map(seat => {
              ...
              return (
                ...
                >
                  ...
                  <rect className="sb"
                    x={cx-tileW/2} y={cy-tileH/2} width={tileW} height={tileH} rx={3}
                    fill={`url(#${fillId})`} stroke={stroke} strokeWidth={0.8}
                    filter="url(#f-shadow)" transform={rot}/>
                  <rect className="ss"
                    x={cx-tileW/2+1.5} y={cy-tileH/2+1.5} width={tileW-3} height={tileH*0.36} rx={2}
                    fill="rgba(255,255,255,0.22)" opacity={isBooked ? 0 : 0.6}
                    style={{ pointerEvents:"none" }} transform={rot}/>
                  {!isBooked && (
            ...
            {seats.length === 0 && Object.entries(POSITION_MAP).map(([sn, pos]) => {
              ...
              return (
                <rect key={sn}
                  x={cx-pos.tileW/2} y={cy-pos.tileH/2} width={pos.tileW} height={pos.tileH} rx={3}
                  fill="rgba(255,255,255,0.04)" stroke="rgba(255,255,255,0.08)" strokeWidth={0.6}
                  transform={`rotate(${pos.deg+90},${cx},${cy})`}/>
              );
            })}

            {/* Row labels — at the left tip of each arc */}
            {ZONES.flatMap(z => z.rows.map(({ row, radius }) => {
              const tipDeg = -90 - ARC_DEG / 2 - 8;
              const tipRad = (tipDeg * Math.PI) / 180;
              return (
                <text key={z.id+row}
                  x={SVG_CX + Math.cos(tipRad) * radius}
                  y={SVG_CY + Math.sin(tipRad) * radius}
            ...
            {ZONES.map(z => {
              ...
              const bx = SVG_CX, by = SVG_CY - mr.radius;
            ...
            {/* Entrance */}
            <text x={SVG_CX} y={SVG_CY+500} textAnchor="middle" fill="rgba(255,255,255,0.55)"
              style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:13, letterSpacing:"0.3em" }}>▲</text>
            <text x={SVG_CX} y={SVG_CY+518} textAnchor="middle" fill="rgba(255,255,255,0.22)"
            ...
            {tooltip && (() => {
              ...
              const W = 96, H = 46;
              const tx = Math.min(Math.max(cx, W/2+12), 940-W/2-12);
              const ty = cy - 30;
              return (
                <g style={{ pointerEvents:"none" }}>
                  <rect x={tx-W/2} y={ty-H} width={W} height={H} rx={6}
                    fill="#0b0b1e" stroke={z.color.border} strokeWidth={1.2} opacity={0.97}/>
                  <rect x={tx-W/2} y={ty-H} width={5} height={H} rx={0} fill={z.color.base} opacity={0.85}/>
                  <text x={tx-W/2+12} y={ty-H+13} fill="#fff"
                    style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:14, letterSpacing:"0.08em" }}>
                    Seat {seat.seatNumber}
                  </text>
                  <text x={tx-W/2+12} y={ty-H+26} fill={z.color.lit}
                    style={{ fontFamily:"'Outfit',sans-serif", fontSize:9, fontWeight:600 }}>
                    {z.label}
                  </text>
                  {seat.price && (
                    <text x={tx-W/2+12} y={ty-H+38} fill="rgba(255,255,255,0.65)"
                      style={{ fontFamily:"'Outfit',sans-serif", fontSize:9, fontWeight:500 }}>
        ...
        <div className="legend">
          ...
          <div className="legend-item">
            <div className="legend-sq" style={{ background: "#1e1e30", border: "1px solid #252540" }}/>
            Unavailable
          </div>
          <div className="legend-item">
