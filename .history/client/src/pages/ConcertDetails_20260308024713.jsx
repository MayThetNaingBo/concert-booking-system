...
function seatsForRing(radius, tileW) {
  return Math.max(2, Math.floor(((ARC * Math.PI) / 180 * radius) / (tileW + GAP)));
}
  ...
  ZONES.forEach(zone => {
    zone.rows.forEach(({ row, radius, tileW, tileH }) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const nearLeft  = Math.abs(normDeg - 270) < AISLE_DEG;
        const nearRight = Math.abs(normDeg - 90)  < AISLE_DEG;
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sg:hover .sb { filter:brightness(1.4) saturate(1.3); }
      ...
      <div className="cd-page">
        ...
        <div style={{ display:"flex",justifyContent:"center" }}>
          <svg viewBox="0 0 940 980" width="min(940px,98vw)" height="min(980px,98vw)" style={{ display:"block",overflow:"visible" }}>
            <defs>
              {ZONES.map(z => (
            ...
            {ZONES.map((z,zi) => {
              const o=z.rows[z.rows.length-1].radius+24, i2=zi===0?94:ZONES[zi-1].rows[ZONES[zi-1].rows.length-1].radius+8;
            ...
            {ZONES.flatMap(z=>z.rows.map(r=>(
              <circle key={z.id+r.row} cx={SVG_CX} cy={SVG_CY} r={r.radius} fill="none" stroke={z.color.base} strokeWidth={0.7} opacity={0.09} strokeDasharray="3 9"/>
            )))}

            {ZONES.map((z,zi)=>zi===0?null:(
              <circle key={z.id+"-s"} cx={SVG_CX} cy={SVG_CY} r={z.rows[0].radius-17} fill="none" stroke="rgba(255,255,255,0.08)" strokeWidth={1.2} strokeDasharray="5 5"/>
            ))}

            {[-90,90].map(a=>{const r=(a*Math.PI)/180; return <line key={a} x1={SVG_CX} y1={SVG_CY} x2={SVG_CX+Math.cos(r)*490} y2={SVG_CY+Math.sin(r)*490} stroke="rgba(0,0,0,0.55)" strokeWidth={28}/>;})}
            {[-90,90].map(a=>{const r=(a*Math.PI)/180; return <line key={a+"h"} x1={SVG_CX} y1={SVG_CY} x2={SVG_CX+Math.cos(r)*490} y2={SVG_CY+Math.sin(r)*490} stroke="rgba(255,255,255,0.02)" strokeWidth={28}/>;})}
            ...
            {seats.map(seat => {
              ...
              if (!z) return null;
              const state=getSeatColor(seat), isBooked=state==="booked", isSel=state==="selected", isHov=hovered===seat._id;
              ...
              return (
                <g key={seat._id} className={isBooked?"":"sg"} style={{cursor:isBooked?"not-allowed":"pointer"}}
                  filter={useGlow?"url(#f-glow)":undefined} opacity={isBooked?0.42:1}
                  onClick={()=>handleSeatClick(seat)}
                  onMouseEnter={()=>{if(!isBooked){setHovered(seat._id);setTooltip({seat,cx,cy,z});}}}
                  onMouseLeave={()=>{setHovered(null);setTooltip(null);}}>
                  {isSel&&<rect x={cx-tileW/2-4} y={cy-tileH/2-4} width={tileW+8} height={tileH+8} rx={4} fill="rgba(232,255,71,0.16)" stroke="rgba(232,255,71,0.45)" strokeWidth={1} transform={rot}/>}
                  <rect className="sb" x={cx-tileW/2} y={cy-tileH/2} width={tileW} height={tileH} rx={3} fill={`url(#${fillId})`} stroke={stroke} strokeWidth={0.8} filter="url(#f-shadow)" transform={rot}/>
                  <rect className="ss" x={cx-tileW/2+1.5} y={cy-tileH/2+1.5} width={tileW-3} height={tileH*0.36} rx={2} fill="rgba(255,255,255,0.22)" opacity={isBooked?0:0.6} style={{pointerEvents:"none"}} transform={rot}/>
                  {!isBooked&&<text x={cx} y={cy+0.5} textAnchor="middle" dominantBaseline="middle" fill={textFill} style={{fontFamily:"'Outfit',sans-serif",fontSize:tileW*0.40,fontWeight:800,letterSpacing:"-0.02em",pointerEvents:"none",userSelect:"none"}}>{seat.seatNumber.replace(/^[A-Za-z]+/,"")}</text>}
                </g>
              );
            })}
            ...
            {seats.length===0&&Object.entries(POSITION_MAP).map(([sn,pos])=>{
              const cx=SVG_CX+pos.x, cy=SVG_CY+pos.y;
              return <rect key={sn} x={cx-pos.tileW/2} y={cy-pos.tileH/2} width={pos.tileW} height={pos.tileH} rx={3} fill="rgba(255,255,255,0.04)" stroke="rgba(255,255,255,0.08)" strokeWidth={0.6} transform={`rotate(${pos.deg+90},${cx},${cy})`}/>;
            })}

            {ZONES.flatMap(z=>z.rows.map(({row,radius})=>{
              const tipDeg=-90-ARC/2-8, tipRad=(tipDeg*Math.PI)/180;
              return <text key={z.id+row} x={SVG_CX+Math.cos(tipRad)*radius} y={SVG_CY+Math.sin(tipRad)*radius} textAnchor="middle" dominantBaseline="middle" fill={z.color.base} opacity={0.7} style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:11,letterSpacing:"0.08em"}}>{row}</text>;
            }))}

            {ZONES.map(z=>{
              const mr=z.rows[Math.floor(z.rows.length/2)], bx=SVG_CX, by=SVG_CY-mr.radius, bw=z.id==="VIP"?48:60;
              return (
                <g key={z.id+"-b"}>
                  <rect x={bx-bw/2} y={by-11} width={bw} height={18} rx={9} fill={z.color.dark} stroke={z.color.border} strokeWidth={1} opacity={0.92}/>
                  <text x={bx} y={by+1} textAnchor="middle" dominantBaseline="middle" fill={z.color.lit} opacity={0.95} style={{fontFamily:"'Outfit',sans-serif",fontSize:8.5,fontWeight:800,letterSpacing:"0.14em",pointerEvents:"none"}}>{z.id==="VIP"?"✦ VIP":z.id}</text>
                </g>
              );
            })}
            ...
            <text x={SVG_CX} y={SVG_CY-12} textAnchor="middle" fill="#e8ff47" style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:17,letterSpacing:"0.32em"}}>STAGE</text>
            <text x={SVG_CX} y={SVG_CY+14} textAnchor="middle" fill="rgba(232,255,71,0.65)" style={{fontFamily:"'Outfit',sans-serif",fontSize:16}}>🎤</text>

            <text x={SVG_CX} y={SVG_CY+500} textAnchor="middle" fill="rgba(255,255,255,0.5)" style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:13,letterSpacing:"0.3em"}}>▲</text>
            <text x={SVG_CX} y={SVG_CY+518} textAnchor="middle" fill="rgba(255,255,255,0.22)" style={{fontFamily:"'Outfit',sans-serif",fontSize:9.5,fontWeight:700,letterSpacing:"0.26em"}}>MAIN ENTRANCE</text>

            {tooltip&&(()=>{
              const {seat,cx,cy,z}=tooltip, W=96,H=46;
        ...
        <div className="legend">
          {ZONES.map(z=>(<div key={z.id} className="legend-item"><div className="legend-sq" style={{background:z.color.base,border:`1px solid ${z.color.border}`}}/>{z.label}</div>))}
      ...
      <div className="booking-bar">
        ...
        <div className="booking-right">
          ...
          <button className="booking-btn" disabled={selectedSeats.length===0}>Confirm Booking →</button>
        </div>
      </div>
    </>
  );
