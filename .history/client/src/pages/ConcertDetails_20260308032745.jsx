...
/*
  ...
  Arc = 320°, entrance gap = 40° centred at 270° (TOP — stage is at centre,
  entrance is at the BOTTOM of the bowl, gap faces down).

  Seats spread symmetrically LEFT and RIGHT from the bottom:
    Gap centred at 90° (bottom), so seats run from 110° → 430° (=70°)
    going clockwise — this means they wrap around the TOP of the stage evenly.

  The SVG_CX / SVG_CY is the STAGE CENTRE. The viewBox is sized so the
  outermost ring fits with equal padding on all sides.
*/

const GAP_DEG   = 40;
const ARC_DEG   = 300 - GAP_DEG;          // 320°
// Gap centred at 90° (bottom). Seats start just clockwise of bottom-right.
const START_DEG = 90 + GAP_DEG / 2;       // 110°
const SEAT_GAP  =10;                       // px between tiles

const ZONES = [
  {
    id: "VIP", label: "VIP Floor",
    rows: [
      { row: "A", radius: 120, tileW: 30, tileH: 25 },
      { row: "B", radius: 153, tileW: 28, tileH: 25 },
      { row: "C", radius: 186, tileW: 28, tileH: 25 },
    ],
  ...
  },
  {
    ...
    rows: [
      { row: "D", radius: 230, tileW: 30, tileH: 25 },
      { row: "E", radius: 263, tileW: 30, tileH: 25 },
      { row: "F", radius: 296, tileW: 30, tileH: 25 },
    ],
  ...
  },
  {
    ...
    rows: [
      { row: "G", radius: 340, tileW: 30, tileH: 25 },
      { row: "H", radius: 373, tileW: 30, tileH: 25 },
      { row: "I", radius: 406, tileW: 30, tileH: 25 },
    ],
  ...
  },
  {
    ...
    rows: [
      { row: "J", radius: 454, tileW: 30, tileH: 25 },
      { row: "K", radius: 487, tileW: 30, tileH: 25 },
    ],
    color: { base: "#7C4DFF", lit: "#B388FF", dark: "#4527A0", text: "#fff", border: "#5E35B1" },
  },
];

// Outermost radius + padding → used to size viewBox symmetrically
const OUTER_R  = 454;
const PADDING  = 36;
const ENTRANCE_EXTRA = 52; // extra space at bottom for entrance label

// Stage centre sits exactly in the middle horizontally,
// and (OUTER_R + PADDING) from the top — giving equal left/right/top space.
// Bottom gets entrance label room.
const SVG_SIZE = (OUTER_R + PADDING) * 2;           // 980
const SVG_CX   = SVG_SIZE / 2;                       // 490
const SVG_CY   = SVG_SIZE / 2;                       // 490
const VB_W     = SVG_SIZE;                            // 980
const VB_H     = SVG_SIZE + ENTRANCE_EXTRA;           // 1032
      ...
      <div className="cd-page">
        ...
        <div style={{ display:"flex", justifyContent:"center" }}>
          <svg
            ...
            width={`min(${VB_W}px, 98vw)`}
            height={`min(${VB_H}px, 98vw)`}
          ...
          >
            ...
            {/* Row labels — placed at 270° (top, 12 o'clock) on each ring */}
            ...
            {/* Entrance — sits below the bowl with equal left/right space */}
