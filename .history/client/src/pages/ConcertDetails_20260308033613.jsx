...
/*
  ...
  KEY FIX for even distribution:
    Use (count + 1) divisions → step = ARC_DEG / (count + 1)
    Seat i is placed at: START_DEG + (i + 1) * step
    This gives equal breathing room at BOTH ends of the arc,
    so seats never crowd into the gap edges.
*/

const GAP_DEG   = 40;
const ARC_DEG   = 360 - GAP_DEG;        // 320°
const START_DEG = 90 + GAP_DEG / 2;     // 110°
const SEAT_GAP  = 10;                    // px between tiles (for count calculation)
...
function buildPositionMap() {
  ...
  ZONES.forEach(zone => {
    zone.rows.forEach(({ row, radius, tileW, tileH }) => {
      ...
      // Use (count + 1) divisions → equal gap at both arc ends
