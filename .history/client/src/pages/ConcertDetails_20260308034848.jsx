...
/*
  ...
  SYMMETRIC ARC:
    Gap of 50° centered at 90° (bottom entrance).
    GAP_START = 115°, GAP_END = 425° (=65° + full rotation).
    Seats are linspace'd across the 310° arc with half-step padding
    at both ends → perfect left-right mirror symmetry.
*/

const GAP_DEG    = 10;
const ARC_DEG    = 360 - GAP_DEG;             // 350°
const GAP_CENTER = 90;
const GAP_START  = GAP_CENTER + GAP_DEG / 2;  // 95°
const GAP_END    = GAP_CENTER - GAP_DEG / 2 + 360; // 445°
const SEAT_GAP   = 7;
...
const ZONES = [
  {
    ...
    rows: [
      { row: "A", radius: 120, tileW: 28, tileH: 22 },
      { row: "B", radius: 150, tileW: 28, tileH: 22 },
      { row: "C", radius: 180, tileW: 28, tileH: 22 },
      { row: "D", radius: 210, tileW: 28, tileH: 22 },
    ],
  ...
  },
  {
    ...
    rows: [
      { row: "E",  radius: 248, tileW: 27, tileH: 22 },
      { row: "F",  radius: 278, tileW: 27, tileH: 22 },
      { row: "G",  radius: 308, tileW: 27, tileH: 22 },
      { row: "H",  radius: 338, tileW: 27, tileH: 22 },
      { row: "H2", radius: 368, tileW: 27, tileH: 22 },
      { row: "H3", radius: 398, tileW: 27, tileH: 22 },
      { row: "H4", radius: 428, tileW: 27, tileH: 22 },
      { row: "H5", radius: 458, tileW: 27, tileH: 22 },
      { row: "H6", radius: 488, tileW: 27, tileH: 22 },
      { row: "H7", radius: 518, tileW: 27, tileH: 22 },
    ],
  ...
  },
  {
    ...
    rows: [
      { row: "I",  radius: 556, tileW: 26, tileH: 21 },
      { row: "J",  radius: 584, tileW: 26, tileH: 21 },
      { row: "K",  radius: 612, tileW: 26, tileH: 21 },
      { row: "L",  radius: 640, tileW: 26, tileH: 21 },
      { row: "L2", radius: 668, tileW: 26, tileH: 21 },
      { row: "L3", radius: 696, tileW: 26, tileH: 21 },
      { row: "L4", radius: 724, tileW: 26, tileH: 21 },
      { row: "L5", radius: 752, tileW: 26, tileH: 21 },
    ],
  ...
  },
  {
    ...
    rows: [
      { row: "M",  radius: 790, tileW: 25, tileH: 20 },
      { row: "N",  radius: 818, tileW: 25, tileH: 20 },
      { row: "O",  radius: 846, tileW: 25, tileH: 20 },
      { row: "P",  radius: 874, tileW: 25, tileH: 20 },
      { row: "Q",  radius: 902, tileW: 25, tileH: 20 },
      { row: "R",  radius: 930, tileW: 25, tileH: 20 },
      { row: "S",  radius: 958, tileW: 25, tileH: 20 },
    ],
...
const OUTER_R        = 958;
const PADDING        = 60;
const ENTRANCE_EXTRA = 60;

const SVG_SIZE = (OUTER_R + PADDING) * 2;
const SVG_CX   = SVG_SIZE / 2;
const SVG_CY   = SVG_SIZE / 2;
const VB_W     = SVG_SIZE;
const VB_H     = SVG_SIZE + ENTRANCE_EXTRA;

function seatsForRing(radius, tileW) {
  const arcLen = (ARC_DEG * Math.PI / 180) * radius;
  return Math.max(2, Math.floor(arcLen / (tileW + SEAT_GAP)));
}
...
function buildPositionMap() {
  ...
  ZONES.forEach(zone => {
    zone.rows.forEach(({ row, radius, tileW, tileH }) => {
      ...
      // Linspace with half-step padding at both ends:
      // Divide the arc into (count) equal slots; place each seat at slot centre.
      // slot width = ARC_DEG / count
      // seat[i] angle = GAP_START + (i + 0.5) * slotWidth
      // This guarantees seats[0] and seats[count-1] are equidistant from gap edges
      // → perfect left-right symmetry around 270° (top).
      const slotWidth = ARC_DEG / count;
      for (let i = 0; i < count; i++) {
        const deg = GAP_START + (i + 0.5) * slotWidth;
      ...
      <div className="cd-page">
        ...
        <div style={{ display:"flex", justifyContent:"center", width:"100%" }}>
          <svg
            ...
            width={`min(${VB_W}px, 96vw)`}
            height={`min(${VB_H}px, 96vw)`}
            style={{ display:"block" }}
          >
            <defs>
              {ZONES.map(z => (
            ...
            {/* Bowl */}
            <ellipse cx={SVG_CX} cy={SVG_CY} rx={OUTER_R + 24} ry={OUTER_R + 24} fill="url(#g-floor)"/>
            <ellipse cx={SVG_CX} cy={SVG_CY} rx={OUTER_R + 24} ry={OUTER_R + 24} fill="none" stroke="rgba(255,255,255,0.04)" strokeWidth={2.5}/>
                ...
                >
                  ...
                  {!isBooked && (
                    <text x={cx} y={cy+0.5} textAnchor="middle" dominantBaseline="middle" fill={textFill}
                      style={{ fontFamily:"'Outfit',sans-serif",fontSize:tileW*0.38,fontWeight:800,letterSpacing:"-0.02em",pointerEvents:"none",userSelect:"none" }}>
                      {numLabel}
                    </text>
                  )}
                </g>
            ...
            {/* Entrance */}
            <text x={SVG_CX} y={SVG_CY + OUTER_R + 18} textAnchor="middle" fill="rgba(255,255,255,0.5)"
              style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:13,letterSpacing:"0.3em" }}>▲</text>
            <text x={SVG_CX} y={SVG_CY + OUTER_R + 36} textAnchor="middle" fill="rgba(255,255,255,0.22)"
            ...
            {tooltip && (() => {
              ...
              const W = 96, H = 46;
              const tx = Math.min(Math.max(cx, W/2+12), VB_W-W/2-12), ty = cy - 30;
              return (
