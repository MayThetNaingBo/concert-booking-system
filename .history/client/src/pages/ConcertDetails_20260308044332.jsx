...
const ARC_CY      = STAGE_Y + STAGE_H - 10; // just inside stage bottom

// Arc sweep: seats span from LEFT_DEG to RIGHT_DEG (SVG clockwise from 3-o-clock)
// 90° = bottom, so rows appear below the stage
// We want a ~150° arc centred at 90° (pointing down)
const ARC_HALF    = 75;   // degrees each side of centre
const ARC_START   = 90 - ARC_HALF;  // 15°
const ARC_END     = 90 + ARC_HALF;  // 165°

const AISLE_W     = 44;  // px gap for centre aisle
const TILE_W      = 26;
const TILE_H      = 20;
const SEAT_ARC_GAP = 3;  // degrees between seats (angular gap)
...
// Angular size of one tile at a given radius
function tileAngularSize(radius) {
  return (TILE_W / (2 * Math.PI * radius)) * 360;
}

// Half-angle of the aisle in degrees at a given radius
function aisleHalfAngle(radius) {
  return (AISLE_W / 2 / (2 * Math.PI * radius)) * 360;
}
...
function buildPositionMap() {
  ...
  ZONES.forEach(zone => {
    zone.rows.forEach(({ row, radius }) => {
      const tileAngle  = tileAngularSize(radius);
      const aisleAngle = aisleHalfAngle(radius);
      const step       = tileAngle + SEAT_ARC_GAP * (tileAngle / TILE_W);

      // LEFT side: from centre-aisle going left to ARC_START
      let seatNum = 1;
      // Left seats: angles from (90 - aisleAngle) decreasing toward ARC_START
      const leftSeats = [];
      for (let deg = 90 - aisleAngle - tileAngle / 2; deg >= ARC_START; deg -= step) {
        leftSeats.push(deg);
      }
      // Reverse so seat 1 is closest to aisle
      leftSeats.reverse().forEach(deg => {
        const { x, y } = polarToXY(deg, radius);
        map[`${row}${seatNum}`] = { x, y, deg, zone: zone.id, side: "L" };
        seatNum++;
      });

      // RIGHT side: from centre-aisle going right to ARC_END
      const rightSeats = [];
      for (let deg = 90 + aisleAngle + tileAngle / 2; deg <= ARC_END; deg += step) {
        rightSeats.push(deg);
      }
      rightSeats.forEach(deg => {
        const { x, y } = polarToXY(deg, radius);
    ...
    <>
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .cd-page    { min-height:100vh; padding:4rem 1rem 6rem; font-family:'Outfit',sans-serif; }
      ...
      <div className="cd-page">
        ...
        <div style={{ display:"flex", justifyContent:"center", overflowX:"auto" }}>
          <svg
            ...
            width={`min(${SVG_W}px, 98vw)`}
            height={`min(${SVG_H}px, 88vh)`}
            style={{ display:"block" }}
          >
            <defs>
              {ZONES.map(z => (
