...
const ARC_HALF    = 85;
const ARC_START   = 90 - ARC_HALF;  // 5°
const ARC_END     = 90 + ARC_HALF;  // 175°
...
/*
  Rows exactly matching backend:
  VIP:  A=20
  CAT1: B=40, C=40, D=40
  CAT2: E=30, F=30, G=30, H=30, I=30, J=30
  CAT3: K=20, L=20, M=20, N=20

  seats are split evenly: half on left, half on right of aisle.
  They are spaced evenly across the arc half (ARC_START→90° and 90°→ARC_END)
  with a gap reserved for the aisle at centre.
*/
...
const ZONES = [
  ...
  },
  {
    ...
    rows: [
      { row: "B", seats: 40, radius: 185 },
      { row: "C", seats: 40, radius: 218 },
      { row: "D", seats: 40, radius: 251 },
    ],
  ...
  },
  {
    ...
    rows: [
      { row: "E",  seats: 50, radius: 292 },
      { row: "F",  seats: 50, radius: 322 },
      { row: "G",  seats: 50, radius: 352 },
      { row: "H",  seats: 50, radius: 382 },
      { row: "I",  seats: 50, radius: 412 },
      { row: "J",  seats: 50, radius: 442 },
      { row: "J2", seats: 50, radius: 472 },
      { row: "J3", seats: 50, radius: 502 },
    ],
  ...
  },
  {
    ...
    rows: [
      { row: "K",  seats: 50, radius: 546 },
      { row: "L",  seats: 50, radius: 576 },
      { row: "M",  seats: 50, radius: 606 },
      { row: "N",  seats: 38, radius: 636 },
      { row: "N2", seats: 38, radius: 666 },
      { row: "N3", seats: 38, radius: 696 },
    ],
...
// Build position map using EXACT seat counts from row definition.
// Each row's seats split evenly left/right of centre aisle,
// distributed with linspace (equal angular spacing) across each half-arc.
function buildPositionMap() {
  const map = {};

  ZONES.forEach(zone => {
    zone.rows.forEach(({ row, seats, radius }) => {
      const degPerPx   = 360 / (2 * Math.PI * radius);
      const aisleAngle = (AISLE_W / 2) * degPerPx;

      const leftArcStart  = ARC_START;
      const leftArcEnd    = 90 - aisleAngle;
      const rightArcStart = 90 + aisleAngle;
      const rightArcEnd   = ARC_END;

      const leftCount  = Math.floor(seats / 2);
      const rightCount = seats - leftCount;

      // Linspace with half-slot padding at both ends → perfectly even
      const leftAngles = Array.from({ length: leftCount }, (_, i) =>
        leftArcStart + ((i + 0.5) / leftCount) * (leftArcEnd - leftArcStart)
      );
      const rightAngles = Array.from({ length: rightCount }, (_, i) =>
        rightArcStart + ((i + 0.5) / rightCount) * (rightArcEnd - rightArcStart)
      );

      let seatNum = 1;
      // Left: seat 1 closest to aisle → reverse the angles
      [...leftAngles].reverse().forEach(deg => {
        const { x, y } = polarToXY(deg, radius);
        map[`${row}${seatNum++}`] = { x, y, deg, zone: zone.id, side: "L" };
      });
      rightAngles.forEach(deg => {
        const { x, y } = polarToXY(deg, radius);
        map[`${row}${seatNum++}`] = { x, y, deg, zone: zone.id, side: "R" };
      });
    });
  });
      ...
      <div className="cd-page">
        ...
        <div style={{ display:"flex", justifyContent:"center", overflowX:"auto", width:"100%" }}>
          ...
          >
            ...
            {/* ── All seat positions — zone coloured, always visible ── */}
            {Object.entries(POSITION_MAP).map(([sn, pos]) => {
              const zone     = ZONE_MAP[pos.zone];
              const rot      = `rotate(${pos.deg - 90}, ${pos.x}, ${pos.y})`;
              const seat     = seatMap[sn];
              const isBooked = seat?.isBooked;
              const isSel    = selectedSeats.includes(sn);
              ...
              const numLabel = sn.replace(/^[A-Za-z0-9]*?(\d+)$/, "$1");
              ...
              return (
                <g key={sn}
                  ...
                  onMouseEnter={() => { if (seat && !isBooked) { setHovered(sn); setTooltip({ seat, x: pos.x, y: pos.y, zone }); }}}
                  onMouseLeave={() => { setHovered(null); setTooltip(null); }}
                >
                  {isSel && (
                    <rect x={pos.x - TILE_W/2 - 3} y={pos.y - TILE_H/2 - 3}
                  ...
                  <rect className="sr"
                    ...
                    width={TILE_W} height={TILE_H} rx={3}
                    fill={`url(#${fillId})`} stroke={stroke} strokeWidth={0.8}
                    filter="url(#f-shadow)" transform={rot}/>
                  <rect x={pos.x - TILE_W/2 + 2} y={pos.y - TILE_H/2 + 2}
                    width={TILE_W - 4} height={TILE_H * 0.37} rx={2}
                    fill="rgba(255,255,255,0.18)" transform={rot}
                    style={{ pointerEvents:"none" }}/>
                  {!isBooked && (
                    <text x={pos.x} y={pos.y + 0.5}
                      textAnchor="middle" dominantBaseline="middle" fill={textFill}
                      style={{ fontFamily:"'Outfit',sans-serif", fontSize:8.5, fontWeight:800,
