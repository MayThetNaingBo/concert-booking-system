...
const ZONES = [
  ...
  },
  {
    ...
    rows: [
      { row: "L", radius: 462, maxSeats: 50 },
      { row: "M", radius: 490, maxSeats: 60 },
    ],
...
function buildPositionMap() {
  ...
  ZONES.forEach(zone => {
    zone.rows.forEach(({ row, radius, maxSeats }) => {
      ...
      // If maxSeats is defined, cap each side to half of maxSeats
      if (maxSeats !== undefined) {
        const perSide = Math.floor(maxSeats / 2);
        // Keep only the innermost seats (closest to aisle) = last N of leftSeats, first N of rightSeats
        leftSeats  = leftSeats.slice(-perSide);
        rightSeats = rightSeats.slice(0, perSide);
      }
