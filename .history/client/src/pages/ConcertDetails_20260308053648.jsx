...
const ARC_HALF    = 80;   // default; individual rows may override via arcHalf
const ARC_START   = 90 - ARC_HALF;  // 10°  — used for SVG decorative elements
const ARC_END     = 90 + ARC_HALF;  // 170° — used for SVG decorative elements
...
// arcHalf values pre-calculated so rows K, L, M hit exactly 40, 50, 60 seats:
//   K (r=426): arcHalf=83.12  → 40 seats
//   L (r=462): arcHalf=95.24  → 50 seats
//   M (r=490): arcHalf=107.34 → 60 seats
const ZONES = [
  {
  ...
  },
  {
    ...
    rows: [
      ...
      { row: "I", radius: 370 },
      { row: "J", radius: 398 },
      { row: "K", radius: 426, arcHalf: 83.12 },
    ],
  ...
  },
  {
    ...
    rows: [
      { row: "L", radius: 462, arcHalf: 95.24 },
      { row: "M", radius: 490, arcHalf: 107.34 },
    ],
...
function buildPositionMap() {
  const map = {};
  ZONES.forEach(zone => {
    zone.rows.forEach(({ row, radius, arcHalf: rowArcHalf }) => {
      // Use per-row arcHalf override if provided, otherwise use global ARC_HALF
      const half      = rowArcHalf !== undefined ? rowArcHalf : ARC_HALF;
      const rowStart  = 90 - half;
      const rowEnd    = 90 + half;
      ...
      // LEFT side: start at aisle edge, place seats going left toward rowStart
      const leftSeats = [];
      for (
        let deg = 90 - aisleAngle - tileAngle / 2;
        deg - tileAngle / 2 >= rowStart;
        deg -= stepAngle
      ) {
        leftSeats.push(deg);
      }
      // Reverse: seat 1 is closest to aisle
      leftSeats.reverse();

      // RIGHT side: start at aisle edge, place seats going right toward rowEnd
      const rightSeats = [];
      for (
        let deg = 90 + aisleAngle + tileAngle / 2;
        deg + tileAngle / 2 <= rowEnd;
        deg += stepAngle
      ) {
        rightSeats.push(deg);
      }
