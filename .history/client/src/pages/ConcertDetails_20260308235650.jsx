...
// Global arc bounds (used by VIP + CAT1, and for guide lines / labels)
const ARC_HALF    = 80;
...
/*
  CAT2 and CAT3 get an extended LEFT arc so those sections are fuller.
  arcStart is pushed further left (smaller angle) while arcEnd stays the same.
  This asymmetry fills the left block with more seats for those zones.
*/
const ZONES = [
  {
    id: "VIP", label: "VIP Floor",
    arcStart: ARC_START, arcEnd: ARC_END,   // symmetric
    rows: [
      { row: "A", radius: 130 },
      { row: "B", radius: 158 },
      { row: "C", radius: 186 },
  ...
  },
  {
    ...
    arcStart: -20, arcEnd: ARC_END,          // ← left arc extended to -20°
    rows: [
  ...
  },
  {
    ...
    arcStart: -35, arcEnd: ARC_END,          // ← left arc extended even further to -35°
    rows: [
...
function buildPositionMap() {
  const map = {};
  ZONES.forEach(zone => {
    const zArcStart = zone.arcStart;
    const zArcEnd   = zone.arcEnd;
    ...
    zone.rows.forEach(({ row, radius, tileW: rowTileW, gap: rowGap }) => {
      ...
      // LEFT side: uses zone-specific arcStart
      const leftSeats = [];
      for (
        let deg = 90 - aisleAngle - tileAngle / 2;
        deg - tileAngle / 2 >= zArcStart;
        deg -= stepAngle
      ) {
        leftSeats.push(deg);
      }
      leftSeats.reverse(); // seat 1 closest to aisle

      // RIGHT side: uses zone-specific arcEnd (unchanged)
      const rightSeats = [];
      for (
        let deg = 90 + aisleAngle + tileAngle / 2;
        deg + tileAngle / 2 <= zArcEnd;
        deg += stepAngle
      ) {
        rightSeats.push(deg);
      }
      ...
      <div className="cd-page">
        ...
        <div style={{ display:"flex", justifyContent:"center", overflowX:"auto", width:"100%" }}>
          ...
          >
            ...
            {/* ── Zone atmosphere arcs — use each zone's own arcStart/arcEnd ── */}
            {ZONES.map(z => {
              const r1 = z.rows[0].radius - 12;
              const r2 = z.rows[z.rows.length - 1].radius + 12;
              const rMid = (r1 + r2) / 2;
              const thickness = r2 - r1;
              const startRad = z.arcStart * Math.PI / 180;
              const endRad   = z.arcEnd   * Math.PI / 180;
              ...
              const largeArc = (z.arcEnd - z.arcStart) > 180 ? 1 : 0;
              return (
                <path key={z.id + "-band"}
            ...
            {/* ── Row labels — use zone-specific arcStart for left edge ── */}
            {ZONES.flatMap(z => z.rows.map(({ row, radius }) => {
              const leftEdgeDeg = z.arcStart - 4;
