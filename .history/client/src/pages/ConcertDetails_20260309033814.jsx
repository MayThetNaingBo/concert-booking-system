...
/*
  BACKEND seat counts (concertController.js) — must match exactly:
    A → VIP,  20 seats, RM300
    B → VIP,  40 seats, RM300
    C → VIP,  40 seats, RM300
    D → CAT1, 40 seats, RM200
    E → CAT1, 30 seats, RM200
    F → CAT1, 30 seats, RM200
    G → CAT1, 30 seats, RM200
    H → CAT2, 30 seats, RM120
    I → CAT2, 30 seats, RM120
    J → CAT2, 30 seats, RM120
    K → CAT2, 30 seats, RM120
    L → CAT3, 40 seats, RM80
    M → CAT3, 40 seats, RM80

  Each row has a `maxSeats` field that CAPS position generation so the
  frontend never creates more positions than the backend has seats.
  arcHalf is widened per-zone so all seats in the widest row fit on screen.
*/
const ZONES = [
  {
    ...
    arcHalf: 80,
    rows: [
      { row: "A", radius: 130, maxSeats: 20 },
      { row: "B", radius: 158, maxSeats: 40 },
      { row: "C", radius: 186, maxSeats: 40 },
    ],
  ...
  },
  {
    ...
    arcHalf: 92,
    rows: [
      { row: "D", radius: 222, maxSeats: 40 },
      { row: "E", radius: 250, maxSeats: 30 },
      { row: "F", radius: 278, maxSeats: 30 },
      { row: "G", radius: 306, maxSeats: 30 },
    ],
  ...
  },
  {
    ...
    arcHalf: 92,
    rows: [
      { row: "H", radius: 342, maxSeats: 30 },
      { row: "I", radius: 370, maxSeats: 30 },
      { row: "J", radius: 398, maxSeats: 30 },
      { row: "K", radius: 426, maxSeats: 30 },
    ],
  ...
  },
  {
    ...
    arcHalf: 100,
    rows: [
      { row: "L", radius: 462, maxSeats: 40 },
      { row: "M", radius: 490, maxSeats: 40 },
    ],
...
function buildPositionMap() {
  ...
  ZONES.forEach(zone => {
    const arcStart = 90 - zone.arcHalf;
    const arcEnd   = 90 + zone.arcHalf;

    zone.rows.forEach(({ row, radius, maxSeats }) => {
      ...
      // LEFT side: seats going from aisle outward to arcStart
      const leftSeats = [];
      for (
        let deg = 90 - aisleAngle - tileAngle / 2;
        deg - tileAngle / 2 >= arcStart;
        deg -= stepAngle
      ) {
        leftSeats.push(deg);
      }
      leftSeats.reverse(); // seat 1 is closest to aisle

      // RIGHT side: seats going from aisle outward to arcEnd
      const rightSeats = [];
      for (
        let deg = 90 + aisleAngle + tileAngle / 2;
        deg + tileAngle / 2 <= arcEnd;
        deg += stepAngle
      ) {
        rightSeats.push(deg);
      }

      // Merge and HARD-CAP to maxSeats so we never exceed the backend count
      const allDegs = [...leftSeats, ...rightSeats].slice(0, maxSeats);

      allDegs.forEach((deg, i) => {
        const seatNum = i + 1;
...
// Derive arcStart/arcEnd per zone for guide lines & badges
const ZONE_ARCS = {};
ZONES.forEach(z => {
  ZONE_ARCS[z.id] = { arcStart: 90 - z.arcHalf, arcEnd: 90 + z.arcHalf };
});
  ...
  return (
    <>
      ...
      <div className="cd-page">
        ...
        {/* Zone pills — price pulled from first matching backend seat in that zone */}
        <div className="zone-pills">
          {ZONES.map(z => (
            <div key={z.id} className="zone-pill">
              ...
              {z.label}
              {(() => {
                const sample = seats.find(s => POSITION_MAP[s.seatNumber]?.zone === z.id);
                return sample?.price
                  ? <span style={{ color: z.color.base }}>· RM{sample.price}</span>
                  : null;
              })()}
            </div>
          ))}
        ...
        <div className="seat-svg-wrap">
          ...
          >
            ...
            {ZONES.map(z => {
              const { arcStart, arcEnd } = ZONE_ARCS[z.id];
              const r1      = z.rows[0].radius - 14;
              const r2      = z.rows[z.rows.length - 1].radius + 14;
              const rMid    = (r1 + r2) / 2;
              const thickness = r2 - r1;
              const x1 = ARC_CX + Math.cos(arcStart * Math.PI / 180) * rMid;
              const y1 = ARC_CY + Math.sin(arcStart * Math.PI / 180) * rMid;
              const x2 = ARC_CX + Math.cos(arcEnd   * Math.PI / 180) * rMid;
              const y2 = ARC_CY + Math.sin(arcEnd   * Math.PI / 180) * rMid;
              return (
                <path
                  key={z.id + "-band"}
            ...
            {Object.entries(POSITION_MAP).map(([seatNumber, pos]) => {
              ...
              if (!seat) return null;   // backend seat not found for this position — skip
            ...
            {/* ── Row labels (left edge per zone arcStart) ── */}
            {ZONES.flatMap(z => {
              const { arcStart } = ZONE_ARCS[z.id];
              return z.rows.map(({ row, radius }) => {
                const edgeDeg = arcStart - 4;
            ...
            {/* ── Zone badges (right edge) ── */}
            {ZONES.map(z => {
              const { arcEnd } = ZONE_ARCS[z.id];
              const midRow = z.rows[Math.floor(z.rows.length / 2)];
              const bx = ARC_CX + Math.cos((arcEnd + 5) * Math.PI / 180) * midRow.radius;
              const by = ARC_CY + Math.sin((arcEnd + 5) * Math.PI / 180) * midRow.radius;
                ...
                <g key={z.id + "-badge"}>
                  ...
                  <text
                    ...
                    style={{
                      ...
                      letterSpacing: "0.13em", pointerEvents: "none"
                    }}
              ...
              return (
                <g style={{ pointerEvents: "none" }}>
                  ...
                  {seat.price && (
                    ...
                    >RM {Number(seat.price).toLocaleString()}</text>
                  )}
                </g>
              );
            })()}
        ...
        <div className="booking-right">
          {selectedSeats.length > 0 && (
            <div>
              ...
              <div className="booking-info-total">RM {totalPrice.toLocaleString()}</div>
            </div>
          )}
          <button
